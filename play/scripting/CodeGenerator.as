package scripting
{
   public class CodeGenerator extends Object
   {
       
      private var code:Array;
      
      private var stackList:Array;
      
      private var localVariableList:Array;
      
      public var vmtarget:VirtualMachine;
      
      public function CodeGenerator()
      {
         super();
         this.initialize();
      }
      
      public function initialize() : *
      {
         this.code = new Array();
         this.stackList = new Array();
         this.localVariableList = new Array();
         this.put(null);
         this.beginNewScope();
      }
      
      private function error(param1:String) : *
      {
         throw new Error("CodeGenerator [error] " + param1);
      }
      
      public function getCode() : Array
      {
         return this.code;
      }
      
      public function put(param1:*) : Number
      {
         return this.code.push(param1) - 1;
      }
      
      public function putStoreStack() : *
      {
         this.stackList.push(this.put(null));
      }
      
      public function putLoadStack() : *
      {
         var _loc3_:* = undefined;
         var _loc1_:Array = this.code;
         var _loc2_:* = Number(this.stackList.pop());
         var _loc4_:* = this.put(null);
         while(true)
         {
            _loc3_ = _loc1_[_loc2_];
            _loc1_[_loc2_] = _loc4_;
            if(_loc3_ == null)
            {
               break;
            }
            _loc2_ = _loc3_;
         }
      }
      
      public function popStack() : Number
      {
         return Number(this.stackList.pop());
      }
      
      public function pushStack(param1:*) : *
      {
         this.stackList.push(param1);
      }
      
      public function setStackPatch(param1:*) : *
      {
         if(this.code[this.stackList[this.stackList.length - 1]] != null)
         {
            this.setStackPatchRecursive(this.code[this.stackList[this.stackList.length - 1]],param1);
         }
         else
         {
            this.code[this.stackList[this.stackList.length - 1]] = param1;
         }
      }
      
      public function setStackPatchRecursive(param1:*, param2:*) : *
      {
         if(this.code[param1] == null)
         {
            this.code[param1] = param2;
         }
         else
         {
            this.setStackPatchRecursive(this.code[param1],param2);
         }
      }
      
      public function putCrossLoadStack() : *
      {
         this.swapStack(undefined,undefined);
         this.putLoadStack();
         this.putLoadStack();
      }
      
      public function swapStack(param1:*, param2:*) : *
      {
         var param1:* = this.stackList.length - (param1 !== undefined?param1:0) - 1;
         var param2:* = this.stackList.length - (param2 !== undefined?param2:1) - 1;
         var _loc3_:* = this.stackList[param1];
         this.stackList[param1] = this.stackList[param2];
         this.stackList[param2] = _loc3_;
      }
      
      public function getStackLength() : Number
      {
         return this.stackList.length;
      }
      
      public function cleanUpStack(param1:*) : *
      {
         if(param1 === undefined)
         {
            var param1:* = 0;
         }
         var _loc2_:Array = this.stackList;
         while(_loc2_.length > param1)
         {
            this.popAndDestroyStack();
         }
      }
      
      public function popAndDestroyStack() : *
      {
         var _loc3_:* = undefined;
         var _loc1_:Array = this.code;
         var _loc2_:* = Number(this.stackList.pop());
         while(true)
         {
            _loc3_ = _loc1_[_loc2_];
            _loc1_[_loc2_] = 0;
            if(_loc3_ == null)
            {
               break;
            }
            _loc2_ = _loc3_;
         }
      }
      
      public function putLabel(param1:Label) : *
      {
         if(param1.isExists)
         {
            this.put(param1.address);
         }
         else
         {
            param1.address = this.put(param1.address);
         }
      }
      
      public function setLabel(param1:Label) : *
      {
         var _loc2_:* = this.code.length;
         this.setLabelAddress(param1,_loc2_);
         param1.commitAddress(_loc2_);
      }
      
      public function setLabelAddress(param1:Label, param2:*) : *
      {
         var _loc5_:* = undefined;
         var _loc3_:Array = this.code;
         var _loc4_:* = param1.address;
         while(_loc4_ != null)
         {
            _loc5_ = _loc3_[_loc4_];
            _loc3_[_loc4_] = param2;
            _loc4_ = _loc5_;
         }
      }
      
      public function beginNewScope() : *
      {
         this.localVariableList.unshift(new Object());
      }
      
      public function closeScope() : *
      {
         this.localVariableList.shift();
      }
      
      public function isLocalVariable(param1:*) : Boolean
      {
         return this.localVariableList[0].hasOwnProperty(String(param1));
      }
      
      public function addLocalVariable(param1:*) : *
      {
         this.localVariableList[0][param1] = true;
      }
      
      public function putExpressionResult(param1:ExpressionResult) : *
      {
         switch(param1.type)
         {
            case "variable":
               this.putGetVariable(String(param1.value));
               param1.setType("stack");
               break;
            case "member":
               this.putGetMember(param1.getObjectExpression(),param1.getMemberExpression());
               param1.setType("stack");
               break;
         }
      }
      
      private function putValue(param1:ExpressionResult) : *
      {
         switch(param1.type)
         {
            case "literal":
               this.put(param1.value);
               break;
            case "stack":
               this.putLoadStack();
               break;
            default:
               this.error("putValueError");
         }
      }
      
      private function putBinaryValue(param1:ExpressionResult, param2:ExpressionResult) : *
      {
         if(param1.isType("literal") && param2.isType("literal"))
         {
            this.put(param1.value);
            this.put(param2.value);
         }
         else if(param1.isType("stack") && param2.isType("stack"))
         {
            this.putCrossLoadStack();
         }
         else if(param1.isType("stack"))
         {
            this.putLoadStack();
            this.put(param2.value);
         }
         else if(param2.isType("stack"))
         {
            this.put(param1.value);
            this.putLoadStack();
         }
         else
         {
            this.error("putBinaryValueError");
         }
      }
      
      public function putSuspend() : *
      {
         this.put(this.vmtarget == null?"SPD":this.vmtarget.SPD);
      }
      
      public function putLiteral(param1:ExpressionResult) : *
      {
         this.put(this.vmtarget == null?"LIT":this.vmtarget.LIT);
         this.put(param1.value);
         this.putStoreStack();
      }
      
      public function putCall(param1:ExpressionResult, param2:*) : *
      {
         if(this.isLocalVariable(param1.value))
         {
            this.put(this.vmtarget == null?"CALLL":this.vmtarget.CALLL);
         }
         else
         {
            this.put(this.vmtarget == null?"CALL":this.vmtarget.CALL);
         }
         this.put(param1.value);
         this.put(param2);
         this.putStoreStack();
      }
      
      public function putCallMember(param1:ExpressionResult, param2:ExpressionResult, param3:*) : *
      {
         this.put(this.vmtarget == null?"CALLM":this.vmtarget.CALLM);
         this.putBinaryValue(param1,param2);
         this.put(param3);
         this.putStoreStack();
      }
      
      public function putCallFunctor(param1:*) : *
      {
         this.put(this.vmtarget == null?"CALLF":this.vmtarget.CALLF);
         this.putLoadStack();
         this.put(param1);
         this.putStoreStack();
      }
      
      public function putReturnFunction(param1:ExpressionResult) : *
      {
         this.put(this.vmtarget == null?"RET":this.vmtarget.RET);
         this.putValue(param1);
      }
      
      public function putReturnCoroutine(param1:ExpressionResult) : *
      {
         this.put(this.vmtarget == null?"CRET":this.vmtarget.CRET);
         this.putValue(param1);
      }
      
      public function putFunction() : Label
      {
         var _loc1_:Label = new Label();
         this.put(this.vmtarget == null?"FUNC":this.vmtarget.FUNC);
         this.putLabel(_loc1_);
         this.putStoreStack();
         return _loc1_;
      }
      
      public function putCoroutine() : Label
      {
         var _loc1_:Label = new Label();
         this.put(this.vmtarget == null?"COR":this.vmtarget.COR);
         this.putLabel(_loc1_);
         this.putStoreStack();
         return _loc1_;
      }
      
      public function putArgument(param1:*, param2:String) : *
      {
         this.put(this.vmtarget == null?"ARG":this.vmtarget.ARG);
         this.put(param1);
         this.put(param2);
         this.addLocalVariable(param2);
      }
      
      public function putJump(param1:Label) : *
      {
         this.put(this.vmtarget == null?"JMP":this.vmtarget.JMP);
         this.putLabel(param1);
      }
      
      public function putIf(param1:ExpressionResult, param2:Label) : *
      {
         this.put(this.vmtarget == null?"IF":this.vmtarget.IF);
         this.putValue(param1);
         this.putLabel(param2);
      }
      
      public function putNif(param1:ExpressionResult, param2:Label) : *
      {
         this.put(this.vmtarget == null?"NIF":this.vmtarget.NIF);
         this.putValue(param1);
         this.putLabel(param2);
      }
      
      public function putBinaryOperation(param1:*, param2:ExpressionResult, param3:ExpressionResult) : *
      {
         this.put(param1);
         this.putBinaryValue(param2,param3);
         this.putStoreStack();
      }
      
      public function putUnaryOperation(param1:*, param2:ExpressionResult) : *
      {
         this.put(param1);
         this.putValue(param2);
         this.putStoreStack();
      }
      
      public function putIncrement(param1:ExpressionResult) : *
      {
         this.putIncDec(this.vmtarget == null?"INC":this.vmtarget.INC,false,param1);
      }
      
      public function putDecrement(param1:ExpressionResult) : *
      {
         this.putIncDec(this.vmtarget == null?"DEC":this.vmtarget.DEC,false,param1);
      }
      
      public function putPostfixIncrement(param1:ExpressionResult) : *
      {
         this.putIncDec(this.vmtarget == null?"INC":this.vmtarget.INC,true,param1);
      }
      
      public function putPostfixDecrement(param1:ExpressionResult) : *
      {
         this.putIncDec(this.vmtarget == null?"DEC":this.vmtarget.DEC,true,param1);
      }
      
      private function putIncDec(param1:*, param2:Boolean, param3:ExpressionResult) : *
      {
         var _loc4_:* = undefined;
         var _loc5_:ExpressionResult = null;
         var _loc6_:ExpressionResult = null;
         var _loc7_:String = null;
         switch(param3.type)
         {
            case "member":
               _loc5_ = param3.getObjectExpression();
               _loc6_ = param3.getMemberExpression();
               if(!_loc6_.isLiteral())
               {
                  this.putExpressionResult(_loc6_);
                  _loc4_ = this.popStack();
                  this.putDuplicate(_loc6_);
                  this.pushStack(_loc4_);
                  this.putDuplicate(_loc5_);
                  this.swapStack(1,2);
               }
               else
               {
                  this.putDuplicate(_loc5_);
               }
               this.putGetMember(_loc5_,_loc6_);
               param3.setTypeStack();
               if(param2)
               {
                  this.putDuplicate(param3);
                  _loc4_ = this.popStack();
                  this.putUnaryOperation(param1,param3);
                  this.putSetMember(_loc5_,_loc6_,param3);
                  this.popAndDestroyStack();
                  this.pushStack(_loc4_);
               }
               else
               {
                  this.putUnaryOperation(param1,param3);
                  this.putSetMember(_loc5_,_loc6_,param3);
               }
               break;
            case "variable":
               _loc7_ = String(param3.value);
               this.putGetVariable(_loc7_);
               param3.setTypeStack();
               if(param2)
               {
                  this.putDuplicate(param3);
               }
               this.putUnaryOperation(param1,param3);
               this.putSetVariable(_loc7_,param3);
               if(param2)
               {
                  this.popAndDestroyStack();
               }
               break;
            default:
               this.error("putIncDecError");
         }
      }
      
      public function putWith(param1:ExpressionResult) : *
      {
         this.put(this.vmtarget == null?"WITH":this.vmtarget.WITH);
         this.putValue(param1);
      }
      
      public function putEndWith() : *
      {
         this.put(this.vmtarget == null?"EWITH":this.vmtarget.EWITH);
      }
      
      public function putPush(param1:ExpressionResult) : *
      {
         this.put(this.vmtarget == null?"PUSH":this.vmtarget.PUSH);
         this.putValue(param1);
      }
      
      public function putPop() : *
      {
         this.put(this.vmtarget == null?"POP":this.vmtarget.POP);
         this.putStoreStack();
      }
      
      public function putDuplicate(param1:ExpressionResult) : *
      {
         this.put(this.vmtarget == null?"DUP":this.vmtarget.DUP);
         this.putValue(param1);
         this.putStoreStack();
         this.putStoreStack();
      }
      
      public function putThis() : *
      {
         this.put(this.vmtarget == null?"THIS":this.vmtarget.THIS);
         this.putStoreStack();
      }
      
      public function putArrayLiteral(param1:*) : *
      {
         this.put(this.vmtarget == null?"ARRAY":this.vmtarget.ARRAY);
         this.put(param1);
         this.putStoreStack();
      }
      
      public function putObjectLiteral(param1:*) : *
      {
         this.put(this.vmtarget == null?"OBJ":this.vmtarget.OBJ);
         this.put(param1);
         this.putStoreStack();
      }
      
      public function putGetVariable(param1:String) : *
      {
         if(this.isLocalVariable(param1))
         {
            this.put(this.vmtarget == null?"GETL":this.vmtarget.GETL);
         }
         else
         {
            this.put(this.vmtarget == null?"GET":this.vmtarget.GET);
         }
         this.put(param1);
         this.putStoreStack();
      }
      
      public function putSetVariable(param1:String, param2:ExpressionResult) : *
      {
         if(this.isLocalVariable(param1))
         {
            this.put(this.vmtarget == null?"SETL":this.vmtarget.SETL);
         }
         else
         {
            this.put(this.vmtarget == null?"SET":this.vmtarget.SET);
         }
         this.put(param1);
         this.putValue(param2);
         this.putStoreStack();
      }
      
      public function putSetLocalVariable(param1:String, param2:ExpressionResult) : *
      {
         this.put(this.vmtarget == null?"SETL":this.vmtarget.SETL);
         this.put(param1);
         this.putValue(param2);
         this.putStoreStack();
         this.addLocalVariable(param1);
      }
      
      public function putGetMember(param1:ExpressionResult, param2:ExpressionResult) : *
      {
         if(param2.isType("variable"))
         {
            this.put(this.vmtarget == null?"GETMV":this.vmtarget.GETMV);
            if(param1.isType("literal"))
            {
               this.put(param1.value);
            }
            else
            {
               this.putLoadStack();
            }
            if(this.isLocalVariable(param2.value))
            {
               this.put(this.vmtarget == null?"GETL":this.vmtarget.GETL);
            }
            else
            {
               this.put(this.vmtarget == null?"GET":this.vmtarget.GET);
            }
            this.put(param2.value);
            this.putStoreStack();
         }
         else
         {
            this.put(this.vmtarget == null?"GETM":this.vmtarget.GETM);
            this.putBinaryValue(param1,param2);
            this.putStoreStack();
         }
      }
      
      public function putSetMember(param1:ExpressionResult, param2:ExpressionResult, param3:ExpressionResult) : *
      {
         this.put(this.vmtarget == null?"SETM":this.vmtarget.SETM);
         if(param1.isLiteral())
         {
            if(param2.isLiteral())
            {
               if(param3.isLiteral())
               {
                  this.put(param1.value);
                  this.put(param2.value);
                  this.put(param3.value);
               }
               else
               {
                  this.put(param1.value);
                  this.put(param2.value);
                  this.putLoadStack();
               }
            }
            else if(param3.isLiteral())
            {
               this.put(param1.value);
               this.putLoadStack();
               this.put(param3.value);
            }
            else
            {
               this.put(param1.value);
               this.putCrossLoadStack();
            }
         }
         else if(param2.isLiteral())
         {
            if(param3.isLiteral())
            {
               this.putLoadStack();
               this.put(param2.value);
               this.put(param3.value);
            }
            else
            {
               this.swapStack(undefined,undefined);
               this.putLoadStack();
               this.put(param2.value);
               this.putLoadStack();
            }
         }
         else if(param3.isLiteral())
         {
            this.putCrossLoadStack();
            this.put(param3.value);
         }
         else
         {
            this.swapStack(0,2);
            this.putLoadStack();
            this.putLoadStack();
            this.putLoadStack();
         }
         this.putStoreStack();
      }
      
      public function putNew(param1:*) : *
      {
         this.put(this.vmtarget == null?"NEW":this.vmtarget.NEW);
         this.putLoadStack();
         this.put(param1);
         this.putStoreStack();
      }
      
      public function putDelete(param1:ExpressionResult) : *
      {
         if(param1.isType("variable") || param1.isType("literal"))
         {
            if(this.isLocalVariable(param1.value))
            {
               this.put(this.vmtarget == null?"DELL":this.vmtarget.DELL);
            }
            else
            {
               this.put(this.vmtarget == null?"DEL":this.vmtarget.DEL);
            }
            this.put(param1.value);
            this.putStoreStack();
         }
         else
         {
            this.put(this.vmtarget == null?"DEL":this.vmtarget.DEL);
            this.putLoadStack();
            this.putStoreStack();
         }
      }
      
      public function putDeleteMember(param1:ExpressionResult, param2:ExpressionResult) : *
      {
         this.put(this.vmtarget == null?"DELM":this.vmtarget.DELM);
         this.putBinaryValue(param1,param2);
         this.putStoreStack();
      }
   }
}
